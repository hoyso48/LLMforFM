...
class ModelLoader(ABC):
    ...
    def get_text_embedding(self, text: list[str]) -> torch.Tensor:
        """
        Returns the text embedding of the given text. The resulting vector should be of shape (n_texts, n_features).
        """
...
class CLAPLaionModel(ModelLoader):
    ...
    def __init__(self, type: Literal['audio', 'music'], audio_len: Optional[Union[float, int]] = None):
        ...
        if type == 'audio':
            self.url = 'https://huggingface.co/lukewys/laion_clap/resolve/main/630k-audioset-best.pt'
        elif type == 'music':
            self.url = 'https://huggingface.co/lukewys/laion_clap/resolve/main/music_audioset_epoch_15_esc_90.14.pt'
        elif type == 'fusion':
            self.url = 'https://huggingface.co/lukewys/laion_clap/resolve/main/630k-audioset-fusion-best.pt'
        self.model_file = Path(__file__).parent / ".model-checkpoints" / self.url.split('/')[-1]
    ...
    def patch_model_430(self, file: Path):
        ...
        log.info("Saved patched checkpoint to %s", file)
    ...
    def load_model(self):
        ...
        if not self.model_file.exists():
            self.model_file.parent.mkdir(parents=True, exist_ok=True)
            download_file(self.url, self.model_file)
        ...
        self.model = laion_clap.CLAP_Module(enable_fusion=True if self.type == 'fusion' else False, amodel='HTSAT-base' if self.type == 'music' else 'HTSAT-tiny')
        self.model.load_ckpt(self.model_file)
        self.model.to(self.device)

    def _get_embedding(self, audio: np.ndarray) -> torch.Tensor:
        audio = self.float32_to_int16(audio)
        audio = self.int16_to_float32(audio)
        audio = torch.from_numpy(audio).float().to(self.device)
        if len(audio.shape) == 1:
            audio = audio.unsqueeze(0)
    ...
    def _get_embedding_chunked(self, audio: np.ndarray) -> torch.Tensor: #orignal implementation
...
def get_all_models(audio_len: Optional[Union[float, int]] = None) -> list[ModelLoader]:
    ...
    ms = [
        ...
        CLAPLaionModel('audio', audio_len=audio_len), CLAPLaionModel('music', audio_len=audio_len), CLAPLaionModel('fusion', audio_len=audio_len),
