...
class KernelAudioDistance:
    ...
    def score(self, baseline: PathOrPaths, eval: PathOrPaths):
        ...
        """
        # If the input is a list of files, the cache (which is directory-based) might be stale.
        # Don't use cache if the input is not a directory.
        if isinstance(baseline, (str, Path)) and Path(baseline).is_dir():
            bg_cache_dir = self.get_cache_dir(self.baseline_dir_for_cache)
        else:
            bg_cache_dir = None
        
        if isinstance(eval, (str, Path)) and Path(eval).is_dir():
            eval_cache_dir = self.get_cache_dir(self.eval_dir_for_cache)
        else:
            eval_cache_dir = None
    ...
    def score_inf(self, baseline: PathOrPaths, eval_files: PathOrPaths, steps: int=25, min_n=500, raw: bool=False):
        """
        ...
        embd_bg = self.emb_loader.load_embeddings(baseline)
        bg_cache_dir = self.get_cache_dir(self.baseline_dir_for_cache) if self.baseline_dir_for_cache else None
        cache_dirs = (bg_cache_dir, None)

        eval_files = _get_files(eval_files)
    ...
    def score_individual(self, baseline: PathOrPaths, eval_dir: PathOrPaths, csv_name: Union[Path, str]) -> Path:
        """
        ...
        if isinstance(csv_name, str):
            ...
            csv = Path('data') / 'result' / self.ml.name / "kad-indiv.csv"
        ...
        eval_files = _get_files(eval_dir)
        ...
        scores = tmap(_find_kad_helper, eval_files, desc=f"Calculating scores", max_workers=self.audio_load_worker)

        # 4. Write the sorted scores to csv
        pairs = list(zip(eval_files, scores))
